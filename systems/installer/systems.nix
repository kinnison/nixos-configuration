...
let
  ...
  closure-for-systems =
    let
      ...
      mydeps = builtins.map (i: i.outPath) (builtins.attrValues flakeInputs.self.inputs);
      allFlakes =
        let
          recFlake =
            seen: fname: f:
            let
              ...
              seen' = seen ++ [ fname ];
                ...
                let
                  inputs = if f ? inputs then f.inputs else { };
                in
                [ f.outPath ] ++ (recFlakes seen' inputs);
            in
            if found then [ ] else done;
          recFlakes =
            seen: fis:
            let
              allFlakes = lib.mapAttrsToList (recFlake seen) fis;
            in
            builtins.concatLists allFlakes;
        in
        lib.unique (recFlakes [ ] flakeInputs);
    in
    pkgs.closureInfo { rootPaths = alldeps ++ mydeps ++ allFlakes; };
...
in
{
  ...
  environment.etc."install-closure".source = "${closure-for-systems}/store-paths";
}
